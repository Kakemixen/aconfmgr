...
notice_paths=() # Array for forced files even if some parent ignored
...
# Adds stars to all paths in array in-place
function AconfAddStarToPaths () {
	declare -n _paths="$1"
...
# Adds all unique dirnames to pathnames in-place
# Uses the path itself with the dirname function
function AconfAddDirnamesToPaths() {
	declare -n _notice_paths="$1"
	local notice_dirnames=

	# find unique parent dirs
	local path
	...
	do
		...
		do
			if [[ "$folder" =~ .*"*".* ]]; then
				#FatalError 'Parent folders of paths do not support "*". It expands to all subdirs and files, thus adding all files contained in directory\n' # Bug
				echo "*" in parent folders of paths expand to "[^/]*" to avoid matching subdirectories and such as a part of "*"
				folder=`echo $folder | tr "*" "[^/]*"`
			fi
			if [[ ! " ${notice_dirnames[@]} " =~ " ${folder} " ]]; then
				notice_dirnames+=($folder)
			fi
			folder=`dirname $path`
			path=$folder
		done
	...
	# add to array inplace
	for folder in  ${notice_dirnames[@]}
	do
		_notice_paths+=($folder)
	done
}
...
function AconfCreateFindRegexArgs() {
	...
	then
		...
		ignore_args_var+=(-regex "$( IFS='|' ; echo "${ignore_regexps[*]}" )")
	fi
}
...
function AconfCompileSystem() {
	...
	# remove "/" from notice_paths if present
	if [[ "${notice_paths[@]}" =~ "/" ]]; then
		declare bad_i
		local i
		for i in ${!notice_paths[@]}
		do
			if [[ "/" == "${notice_paths[$i]}" ]]; then
				bad_i+=($i)
			fi
		done
		for i in ${bad_i[@]}
		do
			unset notice_paths[$i]
		done
	fi

	# modify path lists to facilitate correct regex argument creation
	AconfAddStarToPaths ignore_paths # Add star after path to also match on subdirs
	AconfAddStarToPaths notice_paths # Add star after path to also match on subdirs
	notice_paths+=("/") # Added to avoid pruning of / if IgnorePath * present 
	AconfAddDirnamesToPaths notice_paths # Include parent dirs to avoid preemtive pruning

	local -a ignore_args
	local -a notice_args
	AconfCreateFindRegexArgs ignore_args "${ignore_paths[@]}"
	AconfCreateFindRegexArgs notice_args "${notice_paths[@]}"
...
};
{
	...
	(
		# NB: Regular expressions can be generated by AconfCreateFindRegexArgs
